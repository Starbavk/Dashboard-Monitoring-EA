...
def get_per_kantor(df: pd.DataFrame) -> pd.DataFrame:
    return (
        df.groupby("Nama Kantor")
        .agg(
            Total=("Nama Lengkap", "count"),
            Pegawai_V_Aktif=("Pegawai V", lambda x: (x == "Sudah").sum()),
            Pegawai_X_Aktif=("Pegawai X", lambda x: (x == "Sudah").sum()),
        )
        .reset_index()
    )
